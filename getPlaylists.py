...
from parseData import get_video_info, get_channel_info, get_playlists_channel
...
def thumbnail_save(thumbnail_url, target_path) -> pathlib.Path | None:
    #print('🖼 thumbnail_clip')
    if not thumbnail_url:
        print('🛑️ Err \t not thumbnail_url:')
        return
    ...
    return write_file(target_path, r.content, mode='wb+')




def getAllAuthorsDirs():
    return list(filter(lambda file: file.is_dir(), AUTHORS_DIR.iterdir()))


def getPlaylistsForOneAuthor(author_username):
    print('🎭 getPlaylistsForOneAuthor(author_username)')
    print(author_username)

    author_dir = AUTHORS_DIR.joinpath(author_username)

    playlists_url = f'https://www.youtube.com/@{author_username}/playlists?view=1&sort=dd&shelf_id=0'
    print(playlists_url)
    print()

    playlists = get_playlists_channel(playlists_url)





def getPlaylistsForAllAuthors():
    print('🎭🎭 getPlaylistsForAllAuthors()')

    for author_dir in getAllAuthorsDirs():
        getPlaylistsForOneAuthor(author_dir.name)
        print()


if __name__ == '__main__':
    getPlaylistsForAllAuthors()
