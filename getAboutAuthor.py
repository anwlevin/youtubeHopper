...
from parseData import get_video_info, get_channel_info
...
def authorOneGetAbout(author_username):
    print('🎭 authorGetAboutAll')
    print(author_username)

    author_dir = AUTHORS_DIR.joinpath(author_username)

    author_url = f'https://www.youtube.com/@{author_username}'
    print(author_url)
    print()



    data = get_channel_info(author_url)
    data['thumbnail'] = ''
    if data['thumbnail_url']:
        thumbnail = author_dir.joinpath('about-thumbnail.jpg')
        data['thumbnail'] = thumbnail_save(data['thumbnail_url'], thumbnail).relative_to(author_dir).as_posix()

    about_text = yaml.dump(
        data,
        default_flow_style=False,
        sort_keys=False,
        allow_unicode=True,
    )
    write_file(author_dir.joinpath('about.yml'), about_text)
    print()

def authorGetAboutAll():
    print('🎭🎭 authorGetAboutAll')
    author_usernames = list(filter(lambda file: file.is_dir(), AUTHORS_DIR.iterdir()))
    for author in author_usernames:
        authorOneGetAbout(author.name)
